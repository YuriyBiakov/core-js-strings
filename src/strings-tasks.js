...
function removeFirstOccurrences(str, value) {
  ...
  if (startIndex === 0) return str.slice(endIndex);
  return str.slice(0, startIndex).concat(str.slice(endIndex));
}
...
function countVowels(str) {
  ...
  VOVELS.map((elA) => {
    array.map((elB) => {
      result = elA === elB ? (result += 1) : result;
      return result;
    });
    return result;
  });
...
function isPalindrome(str) {
  const regexp = /[.,:;?!-\s]/g;
  const tempString = str.replaceAll(regexp, '').toLowerCase();
  return tempString === reverseString(tempString);
}
...
function reverseWords(str) {
  ...
  let result = '';
  array.forEach((el) => {
    result = result.concat(' ').concat(reverseString(el));
  });
  return result.trim();
}
...
function invertCase(str) {
  ...
  arr.forEach((el) => {
    ...
    } else result.push(el.toLowerCase());
  });
  return result.join('');
}
